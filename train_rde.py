...
from rde_engine import evaluate_full_images, evaluate_instance_level, maybe_save_best, train_one_epoch
from rde_losses import RadialProfileEllipseLoss
from rde_model import RadialProfileEllipseNet
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Radial Profile Ellipse training for IR small targets")
    # dataset
    ...
    # radial profile
    parser.add_argument("--num-profile-samples", type=int, default=36)
    parser.add_argument("--temperature", type=float, default=12.0)

    # two-stage
    parser.add_argument("--pretrain-base", type=str, default=None)
    ...
    parser.add_argument("--w-profile", type=float, default=1.0)
    ...
    parser.add_argument("--output-dir", type=str, default="runs/rde_reconstruction")
    parser.add_argument("--resume", type=str, default=None)
    parser.add_argument("--eval-only", action="store_true")
    return parser.parse_args()


def _load_base_only_checkpoint(model, path, device):
...
def main() -> None:
    ...
    if train_set is None:
        raise ValueError("Training split is unavailable.")
    ...
    criterion = RadialProfileEllipseLoss(
        w_dice=args.w_dice, w_iou=args.w_iou, w_bce=args.w_bce,
        w_param=args.w_param, w_profile=args.w_profile,
        w_moment=args.w_moment, moment_warmup_epochs=args.moment_warmup_epochs,
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_stats = train_one_epoch(
            ...
            device=device, patch_size=args.patch_size,
            num_profile_samples=args.num_profile_samples,
            temperature=args.temperature, current_epoch=epoch,
        )
        scheduler.step()
        ...
        best_score = maybe_save_best(
            checkpoint_dir=out_dir, model=model, optimizer=optimizer,
            epoch=epoch, metrics=record, best_score=best_score, score_key="full_IoU",
        )
