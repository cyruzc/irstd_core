...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Centroid-conditioned ellipse reconstruction for IR small targets")
    ...
    parser.add_argument("--output-dir", type=str, default="runs/ellipse_reconstruction")
...
def main() -> None:
    ...
    model = CentroidConditionedEllipseNet(in_channels=2, base_channels=args.base_channels, hidden_dim=args.hidden_dim).to(device)
    ...
    if args.eval_only:
        instance_metrics = evaluate_instance_level(model, val_loader, device=device, patch_size=args.patch_size, use_amp=args.amp, threshold=args.threshold)
        full_metrics = evaluate_full_images(model, val_loader, device=device, patch_size=args.patch_size, use_amp=args.amp, threshold=args.threshold, distance_thresh=args.distance_thresh)
